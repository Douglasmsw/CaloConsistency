...
# Train a consistency distillation model with

```bash
python3 scripts/distill_diffu.py \
        ...
        --model_loc [PATH TO PRE-TRAINED DIFFUSION WEIGHTS] \
        --num_sample [NUMBER OF STEPS IN MULTI-STEP CONSISTENCY SAMPLING] \
        --ode_solver [ONE OF paper_euler, paper_heun, back_euler, back_direct, or back_heun] 
```
* The above block allow you to load in trained diffusion model and train consistency model 
* There are additional options in `distill_diffu.py` to control training
* All flags are stored in a config file after training for future reference

# Evaluate consistancy distillation model with
```bash
python3 scripts/plot.py \
	--data_folder [PATH TO FOLDER CONTAINING DATASETS] \
	--config [PATH TO CONFIG FILE FOR DATASET AND PARTICLE] \
	--binning_file [PATH TO RELEVANT BINNING FILE IN CaloChallenge DIRECTORY] \
	--model_loc [PATH TO CONSISTENCY MODEL WEIGHTS] \
	--model Consist \
	--sample
```
* The above block allows you to sample from the moel based on initial energies in a separate validation dataset
* There are more options in `plot.py` which should be set to match additional flags from `distill_diffu.py`
  
```bash
cd CaloChallenge/code
python3 evaluate.py \
	--input_file [LOCATION OF DATASET GENERATED BY PLOT.PY] \
	--reference_file [LOCATION OF EVALUATION DATASET NAMED IN CONFIG FILE] \
	--dataset [TYPE OF PARTICLE BEING COMPARED FROM '1-photons', '1-pions', '2', '3'] \
	--output_dir [DESIRED OUTPUT FOLDER FOR EVALUATION RESULTS]
```
* The above block runs the newly sampled dataset and compares it to the distribution of validation dataset
* Plots are stored in `--output_dir`
